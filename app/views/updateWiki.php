                ...
                <body style="overflow: hidden visible;">
                    <div id="page-container" class="flex items-center justify-center h-screen">
                        <div class="bg-white w-full md:w-1/2 p-8 shadow-lg rounded-md">
                            <h1 class="text-3xl font-bold mb-6">Update Article</h1>
                            <form action="index.php?action=updateWiki&id=<?= $wikis->getIdWiki()?>&img=<?= $wikis->getImage() ?>" method="post" enctype="multipart/form-data">
                                <div class="mb-4">
                                    <label class="text-sm font-semibold text-gray-600">Current Image</label>
                                    <img src="asstes/img/<?= $wikis->getImage() ?>" alt="Current Image" class="mt-2 w-20">
                                </div>
                                <div class="mb-4">
                                    <label for="image" class="text-sm font-semibold text-gray-600">Upload New
                                        Image</label>
                                ...
                                <div class="mb-4">
                                    <input hidden type="hidden" id="isActive" name="isActive" value="<?= $wikis->getIsActive() ?>">
                                </div>

                                <div class="mb-4">
                                    <label for="title" class="text-sm font-semibold text-gray-600">Title</label>
                                    <input type="text" id="title" name="title" value="<?= $wikis->getTitre() ?>"
                                        class="w-full px-4 py-2 border rounded-md mt-1 focus:outline-none focus:border-indigo-500"
                                        required>
                                </div>
                                <div class="mb-4">
                                    ...
                                    <textarea id="content" name="content"
                                        class="w-full px-4 py-2 border rounded-md mt-1 focus:outline-none focus:border-indigo-500"
                                        rows="5" required><?= $wikis->getContenu() ?></textarea>
                                </div>
                                <div class="mb-4">
                                    ...
                                    <select id="category" name="category" x-model="newCat" 
                                        ...
                                        <?php foreach ($catsDATA as $cat) { ?>
                                            <option value="<?= $cat->getIdCat() ?>" <?= ($cat->getIdCat() == $wikis->getIdCat()) ? 'selected' : '' ?>>
                                                <?= $cat->getCatName() ?>
                                            </option>
                                        <?php } ?>
                                    </select>
                                ...
                                <div>
                                    <div class="mb-4">
                                        <label for="tags" class="text-sm font-semibold text-gray-600">Select
                                            Tags</label>
                                        <?php foreach ($tagsDATA as $tag) { ?>
                                            <input type="checkbox" name="tags[]" value="<?= $tag->getIdTag() ?>" <?= (in_array($tag->getTagName(), $tagsInWiki)) ? 'checked' : '' ?>>
                                            <?= $tag->getTagName() ?>
                                        <?php } ?>
                                    </div>
                                </div>
