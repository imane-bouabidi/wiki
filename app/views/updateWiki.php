                    ...
                    <body style="overflow: hidden visible;">
                        <div id="page-container" class="flex items-center justify-center h-screen">
                            <div class="bg-white w-full md:w-1/2 p-8 shadow-lg rounded-md">
                                <h1 class="text-3xl font-bold mb-6">Update Wiki</h1>
                                <form
                                    action="index.php?action=updateWiki&id=<?= $wikis->getIdWiki() ?>&img=<?= $wikis->getImage() ?>"
                                    method="post" enctype="multipart/form-data">
                                    <div class="mb-4">
                                        <label class="text-sm font-semibold text-gray-600">Current Image</label>
                                        <img src="asstes/img/<?= $wikis->getImage() ?>" alt="Current Image"
                                            class="mt-2 w-20">
                                    </div>
                                    <div class="mb-4">
                                    ...
                                    <div class="mb-4">
                                        <input hidden type="hidden" id="isActive" name="isActive"
                                            value="<?= $wikis->getIsActive() ?>">
                                    </div>
                                        ...
                                        <select id="category" name="category" x-model="newCat"
                                            ...
                                            <?php foreach ($catsDATA as $cat) { ?>
                                                <option value="<?= $cat->getIdCat() ?>"
                                                    <?= ($cat->getIdCat() == $wikis->getIdCat()) ? 'selected' : '' ?>>
                                                    <?= $cat->getCatName() ?>
                                                </option>
                                            <?php } ?>
                                        </select>
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <?php foreach ($tagsDATA as $tag) { ?>
                                                <input type="checkbox" name="tags[]" value="<?= $tag->getIdTag() ?>"
                                                    <?= (in_array($tag->getTagName(), $tagsInWiki)) ? 'checked' : '' ?>>
                                                <?= $tag->getTagName() ?>
                                            <?php } ?>
                                        </div>
                                    </div>
